...
- **Regex match type** — a `regex` match type takes a raw PCRE pattern and substitutes numeric capture groups (`$1`, `$2`, …) into the destination (e.g. `^blog/(\d+)$` → `news/$1`), alongside the existing `<name>` / `<name:regex>` patterns.
